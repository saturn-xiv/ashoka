...
// ---------------------------
ashoka::redis::Factory::Factory(const std::string host,
...
ashoka::redis::Config::Config()
    : host("127.0.0.1"), port(6379), db(0), pool_size(16) {}
...
std::string ashoka::redis::Config::name() const { return "redis"; }
// ---------------------------
...
ashoka::redis::Config::open()
{
  ...
  std::shared_ptr<ashoka::pool::Pool<ashoka::redis::Connection>> pool(
      new ashoka::pool::Pool<ashoka::redis::Connection>(this->pool_size,
                                                        factory));
  return pool;
}
